...
$(function(){
	...
	if(page_no > 1){
		$("#left_sider_btn").click(function(){
			...
			var page = page_no - 1;
			location.href="./view_container_v3.php?idx="+idx+"&page="+page;		
		});
	}
	...
	$("#calc_btn").click(function(){
		...
		$.ajax({ 
				...
				success: function(data) {
					//console.log(data)
					location.href="./view_container_v3.php?idx="+idx+"&page=1";
				} 
		});	 
...
for(var k=0 ; k < item_arr.length ; k++){
	...
	var pw = (wide + flange) / 10; // product wide 크기의 십분의 1
	var max_cp = Math.floor(cw/ pw); // 컨테이너 wide 크기에서 product wide가 최대 몇개가 들어갈수 있을까 소수점 버림 ;; 이러면 1단의 최대적재 갯수 나옴

	var each_wide_margin = wide / 20; // 2. 컨테이너 왼쪽벽 위치 계산 ;; conwidth 나누기 4  ;;  나누기 4 하는 이유는 컨테이너 중간위치 2 , 그리고 제품 중앙위치가 컨테이너 왼쪽벽에 걸려서 2 
	
	var container_left_location = -(conWidth/2) + each_wide_margin + wide_margin;
	var container_end_location = -conLength/2 + length /2 + length_margin;
	
	var product_wide_size = (wide + flange)  / 10;
	var product_height_size = height / 10;
	var product_length_size = length;
	...
	for(var z=0; z < max_height_stack ; z ++){
		...
		for(var x=0; x < max_cp ; x++){
			...
			t.position.y = 10;
			t.position.z = container_end_location + (k * ( product_length_size + product_interval ));
			scene.add(t);
			...
			p.position.z = container_end_location + (k * ( product_length_size + product_interval ));
	...
	total_product_length = total_product_length +  (k * ( product_length_size + product_interval ) );
...
if(total_product_length == 0){
	total_product_length = product_length_size + 10;
}

var container_reminder_length =  (conLength*10) - (total_product_length*10) - (length_margin*10) ;
