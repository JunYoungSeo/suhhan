...
<link type="text/css" rel="stylesheet" href="view_container2.css">
...
<div id="main_div">
	<div id="header_div">
		<div id="left_header_div">
			<div id="left_header_1_div">
				<a href="../history.php"><img src="./image/bizi4.png" width="60%" style="padding:7% 10%"></a>
			</div>
			<div id="left_header_2_div">
				<div id="left_header_2_1_div">
					Container 사양(Ft)<br>
					<select style="padding:5px 5px;width:75%;margin-top:5%;border:3px solid #0d47a1;" name="container_fit" >
						<option>-</option>
						<option value="20" <?=$container_fit == "20" ? "selected" : ""; ?>>20 Ft</option>
						<option value="40" <?=$container_fit == "40" ? "selected" : ""; ?>>40 Ft</option>
					</select>
				</div>
			</div>
			<div id="left_header_3_div">
				<div id="left_header_3_1_div">
					적재 방식(단)<br>
					<select style="padding:5px 5px;width:75%;margin-top:8%;border:3px solid #0d47a1;" name="lm">
						<option>-</option>
						<option value="1" <?=$lm == "1" ? "selected" : ""; ?>>1단</option>
						<option value="2" <?=$lm == "2" ? "selected" : ""; ?>>2단</option>
					</select>
				</div>
			</div>
			<div id="left_header_4_div">
				<div id="left_header_4_1_div">
					여유율(단위:mm)
					<button id="calc_btn" style="float:right;margin-right:1%;">적용(Calc)</button>
					<br>
					
						<input type="text" style="width:33%;border:3px solid #0d47a1;float:left;padding: 5px 5px;margin-top: 13px;" <?if($h_margin){print "value=".$h_margin;}?> name="h_margin" placeholder="폭(Width/단위:mm)">
						<input type="text" style="width:33%;border:3px solid #0d47a1;float:left;padding: 5px 5px;margin-top: 13px;border-left: none;" <?if($v_margin){print "value=".$v_margin;}?> name="v_margin" placeholder="길이(Length/단위:mm)">
						<input type="text" style="width:33%;border:3px solid #0d47a1;float:left;padding: 5px 5px;margin-top: 10px;border-left: none;" <?if($height_margin){print "value=".$height_margin;}?> name="height_margin" placeholder="높이(Height/단위:mm)">
					
				</div>
			</div>
		</div>
		<div id="right_header_div">
			<div id="right_header_1_div">
				<div id="right_header_1_1_div">
				 빈 공간<br>
				 <input type="text" style="width:100%;border:3px solid #0d47a1;margin-top: 13px;padding: 5px 5px;" name="container_reminder_length" disabled>
				</div>
			</div>
			<div id="right_header_2_div">
				<div id="right_header_2_1_div">
					Page (Container)<br>
					<?if($page > 1){?>
						<button id="left_sider_btn">◀</button>
					<?}?>
					<input type="text" style="width:40%;border:3px solid #0d47a1;margin-top: 13px;padding: 5px 5px;font-weight:bold;font-size:15px;text-align:center;" value="<?=$page." / ".$total_page?>" name="view_page">
					<?if($page != $total_page){?>
			...
			<div id="right_header_3_div">
				<div id="right_header_3_1_div">
					<a href="./download_packinglist.php?idx=<?=$idx?>" style="text-decoration:none;color:black;"><img src="../images/excel.png" width="50%" height="60%" style="vertical-align: middle;"><br>PACKING LIST DOWNLOAD</a>
				</div>
			</div>
		</div>
	</div>
	...
	<div id="left_div">
		<div id="left_1_div" style="display:none;">
			<img src="./image/3dview.png" width="50%" height="40%" style="padding:15% 20%;">
		</div>
	</div>

	<div id="right_div">
			
    <div class="list_div" style="width:100%;height:300px;overflow-y:auto;">
        <table id="list_table" style="width:100%;border:none;border-collapse:collapse;">
            <colgroup>
				<col width="8%">
				<col width="52%">
				<col width="15%">
			...
			<tr style="cursor: default;">
               	<th style="border-right:3px solid white;">No</th>
				<th style="border-right:3px solid white;">사양(단위:mm)</th>
				<th style="border-right:3px solid white;">수량(EA)</th>
				<th style="border-right:3px solid white;">CBM</th>
				<th>중량(Kg)</th>
            </tr>
            <tr>
				<?
				...
				<tr>
					<td colspan="2" style="border-right:3px solid white;">Total</td>
					<td style="border-right:3px solid white;"><?=$total_quantity?></td>
					<td style="border-right:3px solid white;"><?=$total_cbm?></td>
...
$(function(){
	...
	if(page_no > 1){
		$("#left_sider_btn").click(function(){
			...
			var page = page_no - 1;
			location.href="./view_container_v17.php?idx="+idx+"&page="+page;		
		});
	}
	...
	$("#calc_btn").click(function(){
		...
		$.ajax({ 
				...
				success: function(data) {
					//console.log(data)
					location.href="./view_container_v17.php?idx="+idx+"&page=1";
				} 
		});	 
...
$(window).on('load', function(){
  setTimeout(function() {
	 ...
	 $("canvas").css("left","1%");
	 $("canvas").css("width","68%");
	 $("canvas").css("height","83%");
	 $("canvas").css("border","3px solid #0d47a1");
	 $("canvas").css("border-radius","10px");
	 $("canvas").css("z-index","1");
	 $("#left_1_div").css("display","block");
...
var conWidth, conHeight, conLength;
var fov;
...
var deg_arr = new Array("<?=implode("\",\"",$deg_arr);?>");
...
var container_fit = "<?=$container_fit?>";
...
setting_camera(fov,WIDTH,HEIGHT);
setting_light();
setting_renderer(WIDTH,HEIGHT);

function setting_renderer(WIDTH,HEIGHT){
...
create_container_wall(conWidth,conHeight,conLength)
		...
		for(let z=0; z < max_height_stack ; z ++){
			...
			for(let x=0; x < max_cp ; x++){
				...
				switch(deg){
					...
					case "30" : var imageurl = "./image/elbow30.glb";break;
					case "45" : var imageurl = "./image/elbow45.glb";break;
					case "60" : var imageurl = "./image/elbow60.glb";break;
					case "90" : var imageurl = "./image/elbow90.glb";break;
				}
				
				
				loader.load(imageurl, function (gltf) {
...
function setting_camera(fov,WIDTH,HEIGHT){
...
function create_container_wall(conWidth,conHeight,conLength){
