...
<?
	...
	if($idx){
		...
		$QUERY = "SELECT * FROM $db_name WHERE idx = '$idx' and page = '$page' order by FIELD(item,'str','hor') ASC,cbm desc";
		$SQL = $pdo -> prepare($QUERY);
		$SQL -> execute();
		$SQL -> setFetchMode(PDO::FETCH_ASSOC);
...
$(function(){
	...
	if(page_no > 1){
		$("#left_sider_btn").click(function(){
			...
			var page = page_no - 1;
			location.href="./view_container_v6.php?idx="+idx+"&page="+page;		
		});
	}
	...
	$("#calc_btn").click(function(){
		...
		$.ajax({ 
				...
				success: function(data) {
					//console.log(data)
					location.href="./view_container_v6.php?idx="+idx+"&page=1";
				} 
		});	 
...
for(var k=0 ; k < item_arr.length ; k++){
	...
	var item = item_arr[k];
	var wide = wide_arr[k] / 10; // 실제크기의 10분의 1
	var height = height_arr[k] /10; // 실제크기의 10분의 1
	...
	var elbow_rotate = elbow_rotate_arr[k];
	...
	****************/
	...
	var palette_length_size = item_length+20;
	...
	****************/
	...
	var product_height_size = height;
	...
	const outerRadius =  50.1;
	const thetaSegments =  9;
	const phiSegments = 2;
	const thetaStart = 60;
	const thetaLength = Math.PI * 0.72;  

	var product_shape2 = new THREE.RingBufferGeometry(innerRadius, outerRadius,thetaSegments, phiSegments,thetaStart, thetaLength);
	var product_material2 = new THREE.MeshBasicMaterial({color: 0xA9A9A9});
		...
		for(var x=0; x < max_cp ; x++){
				...
				if(z == 5 || z == 11){
					...
					if(item == "str"){
						...
						var p2 = new THREE.Mesh( product_shape, palette_material);

						p2.position.x = product_left_location + (product_wide_size * x ) + middle_location/2;
						p2.position.y = ( palette_height_size ) + (product_height_size/2) + (product_height_size + 1.6)*z;
						p2.position.z = product_end_location + locations2;
					...
					if(item == "str"){
						...
						p.position.y = ( palette_height_size ) + (product_height_size/2) + (product_height_size + 1.6)*z;
						...
						for(var m =0;m<100;m++){
							...
							var p = new THREE.Mesh( product_shape2, product_material2);

							p.rotateX( Math.PI / 2 );
							p.position.x = product_left_location + (product_wide_size * x ) + middle_location/2;
							p.position.y = ( palette_height_size ) + (product_height_size/2) + (product_height_size + 1.6)*z + m/10;
							p.position.z = product_end_location + locations2 + 50;
