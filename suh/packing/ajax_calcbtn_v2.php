...
<?	
	...
	for($i=0;$row = $sql->fetch();$i++){
		...
		array_push($length_arr,$full_package_length."/".$item."/".$wide."/".$height."/".$length."/".$max_product_ea."/".$siderail_thick."/".$weight."/".$radius."/".$deg."/0/0/off");
	...
	for($i=0;$row = $sql->fetch();$i++){
		...
		array_push($length_arr,$full_package_length."/".$item."/".$wide."/".$height."/".$length."/".$max_product_ea."/".$siderail_thick."/".$weight."/".$radius."/".$deg."/".$elbow_x."/".$elbow_y."/".$elbow_rotate);
	...
	for($j = 0;$j < $need_full_package_container_ea ; $j++){
		...
		for($k = 0; $k < count($cutting[$j]); $k++){
			...
			$cbm = cal_cbm($item,$wide,$height,$length,$flange,$elbow_x,$elbow_y,$elbow_rotate,$quantity);
